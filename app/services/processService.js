...
app.factory("processService", function ($http, configService) {
    return {
        addComplexProcess: function(process){
            ...
            return $http.post("http://localhost:8080/dms/process/complex/add",process);
        },
        addPrimitiveProcess: function(process){
            process.company = configService.getConfig().company;
            return $http.post("http://localhost:8080/dms/process/primitive/add",process);
        },
        deleteProcess: function(process){
            return $http.post("http://localhost:8080/dms/process/delete/",process);
        }
    }
});